...
export default function JobSearch() {
  ...
  const fetchJobs = async (resetPage = false) => {
    try {
      ...
      setMatchData(null)
      setSearched(true)
      setShowDetail(false) // reset to list view on new search
    } catch (err) {
      setError("Failed to fetch jobs. Please try again.")
    } finally {
      setLoading(false)
  ...
  const handleSaveJob = async (job) => {
    ...
    try {
      if (isSaved) {
        await api.delete(`/api/saved-jobs/${job.id}`)
        setSavedJobIds(prev => { const u = { ...prev }; delete u[job.id]; return u })
      } else {
        await api.post("/api/saved-jobs", {
          jobId: job.id, title: job.title, company: job.company,
          location: job.location, type: job.type, salary: job.salary,
          description: job.description, applyLink: job.applyLink,
          logo: job.logo, isRemote: job.isRemote, posted: job.posted,
          matchScore: matchData?.matchScore || 0,
        })
        setSavedJobIds(prev => ({ ...prev, [job.id]: { status: 'Saved', matchScore: 0 } }))
      }
  ...
  // Select a job and on mobile show detail panel
  ...
  return (
    <DashboardLayout title="Job Search">
      ...
      <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 mb-3">
        ...
        {/* Location + type row on mobile */}
        <div className="flex gap-2">
          <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg px-3 py-2 flex-1 sm:flex-none">
            <span className="text-[#94A3B8] text-xs">📍</span>
            <input
              value={location}
              onChange={(e) => setLocation(e.target.value)}
              placeholder="Location..."
              className="bg-transparent text-white text-xs placeholder-[#4B5563] focus:outline-none w-full sm:w-24"
            />
          </div>
          <select
            value={type}
            onChange={(e) => setType(e.target.value)}
            className="bg-white/5 border border-white/10 text-[#94A3B8] text-xs rounded-lg px-2 py-2 focus:outline-none flex-shrink-0"
          >
            {JOB_TYPES.map(t => (
              <option key={t} value={t} className="bg-[#111827]">{JOB_TYPE_LABELS[t]}</option>
            ))}
        ...
        {/* Filter + Search buttons row */}
        <div className="flex gap-2">
          <button
            type="button"
            onClick={() => setShowFilters(!showFilters)}
            className={`flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg border transition flex-1 sm:flex-none justify-center ${
              activeFilterCount > 0
                ? "border-[#00D4AA]/50 bg-[#00D4AA]/10 text-[#00D4AA]"
                : "border-white/10 bg-white/5 text-[#94A3B8] hover:text-white"
            }`}
          ...
          <button
            type="submit"
            disabled={loading}
            className="bg-gradient-to-r from-[#00D4AA] to-[#0099FF] text-white text-xs font-semibold px-5 py-2 rounded-lg hover:opacity-90 transition disabled:opacity-60 flex-1 sm:flex-none"
          >
      ...
      {/* Filters Panel */}
      {showFilters && (
        <div className="bg-[#111827] border border-white/10 rounded-xl p-4 mb-3 space-y-4 sm:space-y-0 sm:flex sm:items-center sm:gap-6 sm:flex-wrap">
          <div>
            <p className="text-[#94A3B8] text-[10px] font-medium mb-2">DATE POSTED</p>
            <div className="flex gap-2 flex-wrap">
              {DATE_POSTED_OPTIONS.map(opt => (
          ...
          <div>
            ...
            <button type="button" onClick={() => setRemoteOnly(!remoteOnly)}
              className={`flex items-center gap-2 text-[10px] px-3 py-1.5 rounded-full border transition ${
                remoteOnly ? "border-[#00D4AA] bg-[#00D4AA]/10 text-[#00D4AA]" : "border-white/10 text-[#94A3B8]"
              }`}
            >
              <span className={`w-3 h-3 rounded-full border flex items-center justify-center ${remoteOnly ? "border-[#00D4AA] bg-[#00D4AA]" : "border-[#94A3B8]"}`}>
                {remoteOnly && <span className="text-black text-[8px]">✓</span>}
              </span>
              Remote Only
            </button>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="hidden md:flex gap-4">
            ...
            <div className="w-72 flex-shrink-0">
              <p className="text-[#94A3B8] text-xs mb-3">
                {displayJobs.length} Opportunities · <span className="text-white">Relevance</span>
                {experienceLevel && <span className="text-[#00D4AA]"> · {EXPERIENCE_LEVELS.find(l => l.value === experienceLevel)?.label}</span>}
              </p>
              <div className="space-y-2 max-h-[calc(100vh-280px)] overflow-y-auto pr-1">
                {displayJobs.map((job) => (
                  <div key={job.id}
                    ...
                    className={`p-3 rounded-xl border cursor-pointer transition ${
                      selected?.id === job.id ? "border-[#00D4AA]/50 bg-[#00D4AA]/5" : "border-white/10 bg-[#111827] hover:border-white/20"
                    }`}
                  >
                    <div className="flex items-center justify-between mb-2">
                      {job.logo ? (
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); handleSaveJob(job) }}
                          disabled={saveLoading[job.id]}
                          className="text-sm hover:scale-110 transition-transform"
                        >
                          {saveLoading[job.id] ? <span className="text-[#94A3B8]">...</span>
                            : savedJobIds[job.id] ? <span className="text-[#00D4AA]">🔖</span>
                            : <span className="text-[#94A3B8] hover:text-[#00D4AA]">🔖</span>}
                        </button>
                      </div>
                    </div>
                ...
                {displayJobs.length > 0 && (
                  <button onClick={() => { setPage(p => p + 1); fetchJobs() }}
                    className="w-full text-[#00D4AA] text-xs py-2 border border-[#00D4AA]/20 rounded-lg hover:bg-[#00D4AA]/5 transition mt-2"
                  >Load More</button>
                )}
                {displayJobs.length === 0 && (
            ...
            {!showDetail ? (
                ...
                <div className="space-y-2">
                  {displayJobs.map((job) => (
                    <div key={job.id}
                      ...
                      className="p-3 rounded-xl border border-white/10 bg-[#111827] active:bg-[#00D4AA]/5 cursor-pointer transition"
                    >
                      <div className="flex items-start gap-3">
                        {job.logo ? (
                          <img src={job.logo} alt={job.company} className="w-10 h-10 rounded-lg object-contain bg-white/10 p-0.5 flex-shrink-0" />
                        ) : (
                          <div className="w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center text-sm flex-shrink-0">🏢</div>
                        )}
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-2">
                            <p className="text-white text-sm font-semibold line-clamp-1 flex-1">{job.title}</p>
                            <button onClick={(e) => { e.stopPropagation(); handleSaveJob(job) }}
                              disabled={saveLoading[job.id]}
                              className="text-base flex-shrink-0 p-1"
                            >
                              {saveLoading[job.id] ? <span className="text-[#94A3B8] text-xs">...</span>
                                : savedJobIds[job.id] ? <span className="text-[#00D4AA]">🔖</span>
                                : <span className="text-[#94A3B8]">🔖</span>}
                            </button>
                          </div>
                          <p className="text-[#94A3B8] text-xs">{job.company}</p>
                          <p className="text-[#4B5563] text-[10px] truncate">{job.location}</p>
                          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            {appliedJobs[job.id] && (
                              <span className="text-[#00D4AA] text-[9px] font-bold bg-[#00D4AA]/10 px-2 py-0.5 rounded-full">APPLIED</span>
                            )}
                            {job.isRemote && (
                              <span className="text-[#0099FF] text-[9px] bg-[#0099FF]/10 px-2 py-0.5 rounded-full">REMOTE</span>
                            )}
                            <span className="bg-white/5 text-[#94A3B8] text-[9px] px-2 py-0.5 rounded-full">
                              {JOB_TYPE_LABELS[job.type] || job.type}
                            </span>
                  ...
                  {displayJobs.length > 0 && (
                    <button onClick={() => { setPage(p => p + 1); fetchJobs() }}
                      className="w-full text-[#00D4AA] text-xs py-3 border border-[#00D4AA]/20 rounded-lg hover:bg-[#00D4AA]/5 transition mt-2"
                    >Load More</button>
                  )}

                  {displayJobs.length === 0 && (
                    <div className="text-center py-8">
                      <p className="text-[#94A3B8] text-xs">No jobs found.</p>
                      <button onClick={() => setExperienceLevel("")} className="text-[#00D4AA] text-xs mt-2 hover:underline">
                        Clear experience filter
                      </button>
                    </div>
              ...
              <div>
                ...
                <button
                  ...
                  className="flex items-center gap-2 text-[#94A3B8] text-xs mb-4 hover:text-white transition"
                >
                  ← Back to results
                </button>
                <JobDetail
      ...
      {tailorModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4">
          <div className="bg-[#111827] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[85vh] sm:max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between p-4 sm:p-5 border-b border-white/10">
              <div>
                <h3 className="text-white font-bold text-sm">🎯 Resume Tailoring Tips</h3>
                <p className="text-[#94A3B8] text-xs mt-0.5 truncate">{selected?.title} at {selected?.company}</p>
              </div>
              <button onClick={() => setTailorModal(false)} className="text-[#94A3B8] hover:text-white text-xl transition p-1">✕</button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
              ...
              {tailorError && <p className="text-[#FF4D6D] text-xs text-center py-8">{tailorError}</p>}
              {tailorData && (
                <>
                  <div className="bg-[#0D1321] border border-[#00D4AA]/20 rounded-xl p-3">
                    <p className="text-[#00D4AA] text-[10px] font-bold mb-1">OVERALL FIT</p>
                  ...
                  {tailorData.strengthsToHighlight?.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {tailorData.strengthsToHighlight.map((s, i) => (
                          <span key={i} className="bg-[#00D4AA]/10 text-[#00D4AA] text-[10px] px-3 py-1 rounded-full border border-[#00D4AA]/20">{s}</span>
                        ))}
                      </div>
                    </div>
                  )}
                  {tailorData.keywordsToAdd?.length > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {tailorData.keywordsToAdd.map((k, i) => (
                          <span key={i} className="bg-[#FFB800]/10 text-[#FFB800] text-[10px] px-3 py-1 rounded-full border border-[#FFB800]/20">+ {k}</span>
                        ))}
                      </div>
                    </div>
                  )}
                          ...
                          <div key={i} className="bg-[#0D1321] border border-white/5 rounded-xl p-3">
                            <div className="flex items-center gap-2 mb-1">
                              <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                                s.priority === 'High' ? 'bg-[#FF4D6D]/20 text-[#FF4D6D]' :
                                s.priority === 'Medium' ? 'bg-[#FFB800]/20 text-[#FFB800]' :
                                'bg-white/10 text-[#94A3B8]'}`}>{s.priority}</span>
                              <span className="text-[#94A3B8] text-[10px]">{s.section}</span>
                            </div>
                            <p className="text-white text-xs mb-1">{s.action}</p>
                            <p className="text-[#94A3B8] text-[10px]">{s.reason}</p>
            ...
            <div className="p-4 border-t border-white/10">
              <button onClick={() => setTailorModal(false)}
                className="w-full bg-white/5 border border-white/10 text-white text-xs font-semibold py-2.5 rounded-lg hover:bg-white/10 transition">
                Close
              </button>
            </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 p-4">
          <div className="bg-[#111827] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[85vh] sm:max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between p-4 sm:p-5 border-b border-white/10">
              ...
              <button onClick={() => setCoverLetterModal(false)} className="text-[#94A3B8] hover:text-white text-xl transition p-1">✕</button>
            </div>
            <div className="flex-1 overflow-y-auto p-4 sm:p-5">
              ...
              {coverLetterError && <p className="text-[#FF4D6D] text-xs text-center py-8">{coverLetterError}</p>}
              {coverLetterText && (
                <p className="text-[#94A3B8] text-xs leading-relaxed whitespace-pre-line">{coverLetterText}</p>
              )}
            </div>
            {coverLetterText && (
              <div className="p-4 border-t border-white/10 flex gap-3">
                ...
                <button onClick={() => setCoverLetterModal(false)}
                  className="flex-1 bg-white/5 border border-white/10 text-white text-xs font-semibold py-2.5 rounded-lg hover:bg-white/10 transition">
                  Close
                </button>
              </div>
...
/* ── Shared Job Detail Panel ── */
function JobDetail({
  selected, matchData, matchLoading, matchError,
  savedJobIds, saveLoading, appliedJobs,
  onMatchScore, onSaveJob, onApply, onShare,
  onCoverLetter, onTailorResume,
  JOB_TYPE_LABELS, isMobile = false
}) {
  if (!selected) {
    return (
      <div className="flex-1 bg-[#111827] border border-white/10 rounded-xl flex items-center justify-center min-h-[300px]">
        <p className="text-[#4B5563] text-sm">Select a job to see details</p>
      </div>
    )
  }

  const getScoreColor = (s) => s >= 70 ? '#00D4AA' : s >= 50 ? '#FFB800' : '#FF4D6D'

  return (
    <div className={`${isMobile ? "w-full" : "flex-1"} bg-[#111827] border border-white/10 rounded-xl p-4 sm:p-6 overflow-y-auto max-h-[calc(100vh-280px)]`}>
      {/* Header */}
      <div className="flex items-start justify-between gap-3 mb-4">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          {selected.logo ? (
            <img src={selected.logo} alt={selected.company} className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl object-contain bg-white/10 p-1 flex-shrink-0" />
          ) : (
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white/10 flex items-center justify-center text-xl flex-shrink-0">🏢</div>
          )}
          <div className="min-w-0">
            <h2 className="text-white text-base sm:text-xl font-bold leading-tight">{selected.title}</h2>
            <p className="text-[#94A3B8] text-xs mt-0.5">{selected.company} · {selected.location}</p>
            <p className="text-[#4B5563] text-[10px]">{selected.posted}</p>
          </div>
        </div>
        {/* Badges row — wraps on mobile */}
        <div className="flex items-center gap-1.5 flex-wrap justify-end flex-shrink-0">
          <button onClick={() => onShare(selected)}
            className="flex items-center gap-1 bg-white/5 border border-white/10 text-[#94A3B8] text-[10px] px-2.5 py-1.5 rounded-lg hover:text-white transition">
            📤 Share
          </button>
          <button onClick={() => onSaveJob(selected)} disabled={saveLoading[selected.id]}
            className={`flex items-center gap-1 text-[10px] px-2.5 py-1.5 rounded-lg border transition ${
              savedJobIds[selected.id] ? "border-[#00D4AA]/50 bg-[#00D4AA]/10 text-[#00D4AA]" : "border-white/10 bg-white/5 text-[#94A3B8] hover:text-white"
            }`}>
            {savedJobIds[selected.id] ? "🔖 Saved" : "🔖 Save"}
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {selected.isRemote && <span className="bg-[#00D4AA]/20 text-[#00D4AA] text-xs px-3 py-1 rounded-full">Remote</span>}
        <span className="bg-white/10 text-white text-xs px-3 py-1 rounded-full">{JOB_TYPE_LABELS[selected.type] || selected.type}</span>
      </div>
      ...
      <div className="bg-[#0D1321] border border-[#00D4AA]/20 rounded-xl p-4 mb-4">
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          <p className="text-[#00D4AA] text-xs font-bold">⚡ AI Match Score</p>
          {!matchData && (
            <button onClick={onMatchScore} disabled={matchLoading}
              className="bg-gradient-to-r from-[#00D4AA] to-[#0099FF] text-white text-[10px] font-semibold px-3 py-1.5 rounded-lg hover:opacity-90 transition disabled:opacity-60">
              {matchLoading ? "Analyzing..." : "⚡ Calculate Match"}
            </button>
          )}
          ...
          <div>
            <div className="flex items-center gap-4 mb-3 flex-wrap">
              <div className="w-14 h-14 rounded-full border-4 flex flex-col items-center justify-center flex-shrink-0"
                style={{ borderColor: getScoreColor(matchData.matchScore) }}>
                <p className="font-bold text-sm leading-none" style={{ color: getScoreColor(matchData.matchScore) }}>{matchData.matchScore}%</p>
                <p className="text-[#94A3B8] text-[8px]">MATCH</p>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[#94A3B8] text-xs mb-2">{matchData.summary}</p>
                <div className="flex gap-2 flex-wrap">
                  ...
                  <span className="bg-white/10 text-[#94A3B8] text-[10px] px-2 py-0.5 rounded-full">Exp: {matchData.experienceMatch}</span>
                  <span className="bg-white/10 text-[#94A3B8] text-[10px] px-2 py-0.5 rounded-full">Loc: {matchData.locationMatch}</span>
                </div>
              </div>
            </div>
            {matchData.skillsMatch?.matched?.length > 0 && (
            ...
            <button onClick={onMatchScore} className="mt-3 text-[#94A3B8] text-[10px] hover:text-white transition">🔄 Recalculate</button>
          </div>
        )}
        {!matchData && !matchLoading && (
          <p className="text-[#4B5563] text-xs">Click "Calculate Match" to see how well you match this job.</p>
      ...
      <div className="grid grid-cols-1 xs:grid-cols-3 gap-2 mb-4">
        <a href={selected.applyLink} target="_blank" rel="noreferrer"
          onClick={() => onApply(selected)}
          className="bg-gradient-to-r from-[#00D4AA] to-[#0099FF] text-white text-xs font-semibold py-2.5 rounded-lg hover:opacity-90 transition text-center">
          {appliedJobs[selected.id] ? "✅ Applied" : "🚀 Apply Now"}
        </a>
        <button onClick={onCoverLetter}
          className="bg-white/5 border border-white/10 text-white text-xs font-semibold py-2.5 rounded-lg hover:bg-white/10 transition">
          ✨ Cover Letter
        </button>
        <button onClick={onTailorResume}
          className="bg-white/5 border border-white/10 text-white text-xs font-semibold py-2.5 rounded-lg hover:bg-white/10 transition">
          🎯 Tailor Resume
        </button>
      </div>
      ...
      <div>
        <p className="text-white text-sm font-semibold mb-2">Job Description</p>
        <p className="text-[#94A3B8] text-xs leading-relaxed whitespace-pre-line line-clamp-[20]">{selected.description}</p>
      </div>
      ...
      {selected.highlights?.Qualifications?.length > 0 && (
        <div className="mt-4">
          ...
          <ul className="space-y-2">
            {selected.highlights.Qualifications.slice(0, 5).map((q, i) => (
              <li key={i} className="flex items-start gap-2 text-[#94A3B8] text-xs">
                <span className="text-[#00D4AA] mt-0.5 flex-shrink-0">✓</span>{q}
              </li>
            ))}
          </ul>
        </div>
      ...
      {selected.highlights?.Responsibilities?.length > 0 && (
        <div className="mt-4">
          ...
          <ul className="space-y-2">
            {selected.highlights.Responsibilities.slice(0, 5).map((r, i) => (
              <li key={i} className="flex items-start gap-2 text-[#94A3B8] text-xs">
                <span className="text-[#00D4AA] mt-0.5 flex-shrink-0">→</span>{r}
              </li>
            ))}
          </ul>
        </div>
