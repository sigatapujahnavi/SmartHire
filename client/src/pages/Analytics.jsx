  ...
  return (
    <DashboardLayout title="AI Insights">
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div>
          <h2 className="text-white text-xl sm:text-2xl font-bold">AI Insights</h2>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {/* ── Daily Digest Agent Card ── */}
          <div className="bg-gradient-to-r from-[#00D4AA]/10 to-[#0099FF]/10 border border-[#00D4AA]/30 rounded-xl p-4 sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  <span className="text-xl">⚡</span>
                  <p className="text-white font-bold text-sm sm:text-base">Daily Job Digest Agent</p>
                  <span className="bg-[#00D4AA]/20 text-[#00D4AA] text-[9px] font-bold px-2 py-0.5 rounded-full tracking-widest">
                    AI AGENT
                  </span>
                </div>
                ...
                {/* Agent Status Row — wraps on mobile */}
            ...
            {/* Readiness check — wraps on mobile */}
            <div className="border-t border-white/10 mt-4 pt-3 flex items-center gap-4 sm:gap-6 flex-wrap">
              <div className="flex items-center gap-1.5">
                <span className={profile?.resumeUrl ? "text-[#00D4AA]" : "text-[#FF4D6D]"}>
                  {profile?.resumeUrl ? "✓" : "✗"}
                </span>
              ...
              <p className="text-[#4B5563] text-[10px] sm:ml-auto w-full sm:w-auto mt-1 sm:mt-0">
                Powered by Groq LLaMA 3.3 + JSearch API
              </p>
            </div>
          </div>
          ...
          <div className="bg-[#111827] border border-white/10 rounded-xl overflow-hidden">
            <div className="px-4 sm:px-5 py-4 border-b border-white/10 flex items-center justify-between">
              <div className="flex items-center gap-2">
                <p className="text-[#00D4AA] text-[10px] font-bold tracking-widest">● DIGEST JOB MATCHES</p>
                <span className="text-[#4B5563] text-[10px] hidden sm:inline">Last 3 days</span>
              </div>
              {digestGroups.length > 0 && (
                <span className="text-[#4B5563] text-[10px]">
                  {digestGroups.reduce((sum, g) => sum + g.jobs.length, 0)} total
                </span>
              )}
            </div>
                ...
                {digestGroups.map((group) => {
                  ...
                  return (
                    <div key={group.date}>
                      <button
                        onClick={() => toggleDate(group.date)}
                        className="w-full px-4 sm:px-5 py-3 flex items-center justify-between hover:bg-white/5 transition"
                      >
                        <div className="flex items-center gap-3">
                          {isToday && (
                          ...
                          <span className="text-[#4B5563] text-[10px]">
                            {group.jobs.length} matches
                          </span>
                          <span className="text-[#4B5563] text-[10px] hidden sm:inline">
                            · {group.date}
                          </span>
                        </div>
                        <span className="text-[#4B5563] text-xs">{isExpanded ? "▲" : "▼"}</span>
                      ...
                      {isExpanded && (
                        <div className="px-4 sm:px-5 pb-4 space-y-3">
                          {group.jobs.map((job) => (
                            <div
                              key={job._id}
                              className="bg-[#0D1321] border border-white/5 rounded-xl p-3 sm:p-4 flex items-center justify-between gap-3"
                            >
                              <div className="flex items-center gap-3 flex-1 min-w-0">
                                <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-[#1a2332] flex items-center justify-center flex-shrink-0 overflow-hidden">
                                  {job.logo ? (
                                    <img src={job.logo} alt={job.company} className="w-full h-full object-contain p-1" />
                                  ) : (
                                    <span className="text-white text-xs font-bold">
                                ...
                                <div className="min-w-0">
                                  <p className="text-white text-xs sm:text-sm font-semibold truncate">{job.title}</p>
                                  <p className="text-[#94A3B8] text-[10px] sm:text-xs truncate">
                                    {job.company} · {job.location}
                                  </p>
                                  <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                                    {job.type && <span className="text-[#4B5563] text-[9px] bg-white/5 px-1.5 py-0.5 rounded">{job.type}</span>}
                                    {job.salary && <span className="text-[#4B5563] text-[9px] bg-white/5 px-1.5 py-0.5 rounded hidden sm:inline">{job.salary}</span>}
                                    {job.isRemote && (
                                      <span className="text-[#0099FF] text-[9px] bg-[#0099FF]/10 px-1.5 py-0.5 rounded">Remote</span>
                                    )}
                                  </div>
                                </div>
                              </div>
                              ...
                              <div className="flex flex-col sm:flex-row items-end sm:items-center gap-2 sm:gap-3 flex-shrink-0">
                                <div className="text-right">
                                  <p className="text-base sm:text-lg font-bold" style={{ color: getScoreColor(job.matchScore) }}>
                                    {job.matchScore}%
                                  </p>
                                  <p className="text-[#4B5563] text-[9px]">match</p>
                                </div>
                                <a
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="bg-gradient-to-r from-[#00D4AA] to-[#0099FF] text-white text-[9px] sm:text-[10px] font-bold px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg hover:opacity-90 transition whitespace-nowrap"
                                >
                                  Apply →
                                </a>
                              </div>
          ...
          {/* ── 3 Stats Cards — 1 col on mobile, 3 on desktop ── */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-[#111827] border border-white/10 rounded-xl p-4 sm:p-5">
              ...
              {atsScore > 0 ? (
                <>
                  <div className="flex items-center justify-between mb-4">
                    <p className="text-3xl sm:text-4xl font-bold" style={{ color: getScoreColor(atsScore) }}>
                      {atsScore}<span className="text-sm text-[#94A3B8]">/100</span>
                    </p>
                    <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border-4 flex items-center justify-center"
                      style={{ borderColor: `${getScoreColor(atsScore)}40` }}>
            ...
            <div className="bg-[#111827] border border-white/10 rounded-xl p-4 sm:p-5">
              ...
              {totalJobs > 0 ? (
                <>
                  <div className="flex items-end gap-2 mb-4">
                    <p className="text-white text-3xl sm:text-4xl font-bold">{totalJobs}</p>
                    <p className="text-[#94A3B8] text-xs mb-1">total saved</p>
                  </div>
                  <div className="space-y-2">
                    {Object.entries(statusCounts).map(([status, count]) => {
            ...
            <div className="bg-[#111827] border border-white/10 rounded-xl p-4 sm:p-5">
              ...
              {scoredJobs.length > 0 ? (
                <>
                  <div className="mb-4">
                    <p className="text-[#94A3B8] text-[10px] mb-1">AVG MATCH SCORE</p>
                    <p className="text-3xl sm:text-4xl font-bold" style={{ color: getScoreColor(avgMatchScore) }}>
                      {avgMatchScore}<span className="text-sm text-[#94A3B8]">%</span>
                    </p>
                  </div>
                  <div className="space-y-3">
          ...
          {/* ── Skills + Recommendations — 1 col on mobile, 2 on desktop ── */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-[#111827] border border-white/10 rounded-xl p-4 sm:p-5">
              <div className="flex items-center justify-between mb-3">
                <p className="text-[#00D4AA] text-[10px] font-bold tracking-widest">● SKILLS DETECTED</p>
                <span className="text-[#4B5563] text-[10px]">{topSkills.length} skills</span>
              </div>
              {topSkills.length > 0 ? (
                <div className="flex flex-wrap gap-2">
