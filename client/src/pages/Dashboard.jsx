  ...
  return (
    <DashboardLayout title="Dashboard">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-[#111827] border border-white/10 rounded-xl p-3 sm:p-4">
            <div className="flex items-center justify-between mb-2">
              <p className="text-[#94A3B8] text-[9px] sm:text-[10px] font-medium tracking-wide leading-tight">{stat.label}</p>
              <span className="text-sm">{stat.icon}</span>
            </div>
            <div className="flex items-end gap-1 sm:gap-2">
              <p className="text-white text-2xl sm:text-3xl font-bold">{stat.value}</p>
              <span className="text-[10px] sm:text-xs mb-1 truncate" style={{ color: stat.color }}>
                {stat.change}
              </span>
            </div>
            {stat.sub && <p className="text-[#94A3B8] text-[9px] sm:text-xs mt-1 leading-tight">{stat.sub}</p>}
          </div>
        ))}

        {/* ATS Score */}
        <div className="bg-[#111827] border border-white/10 rounded-xl p-3 sm:p-4 flex items-center justify-between">
          <div className="flex-1 min-w-0">
            <p className="text-[#94A3B8] text-[9px] sm:text-[10px] font-medium tracking-wide mb-1">ATS SCORE</p>
            {loading ? (
              <p className="text-[#94A3B8] text-2xl font-bold">—</p>
            ) : atsScore > 0 ? (
              <>
                <p className="text-[#00D4AA] text-2xl sm:text-3xl font-bold">
                  {atsScore}
                  <span className="text-xs sm:text-sm text-[#94A3B8]">/100</span>
                </p>
                {atsPercentile && (
                  <p className="text-[#00D4AA] text-[10px] sm:text-xs mt-1">{atsPercentile}</p>
                )}
              </>
            ) : (
              <div>
                <p className="text-[#94A3B8] text-xs mt-1">Not analyzed</p>
                <button
                  onClick={() => navigate("/resume")}
                  className="text-[#00D4AA] text-[10px] mt-1 hover:underline"
                >
          ...
          <div className="w-10 h-10 sm:w-14 sm:h-14 rounded-full border-4 border-[#00D4AA]/30 flex items-center justify-center flex-shrink-0">
            <span className="text-[#00D4AA] text-base sm:text-lg">🎯</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Recent Activity */}
        <div className="lg:col-span-2 bg-[#111827] border border-white/10 rounded-xl p-4 sm:p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-white font-semibold text-sm">Recent Activity</h2>
            <button
              onClick={() => navigate("/applications")}
              className="text-[#00D4AA] text-xs hover:underline whitespace-nowrap"
            >
              View All →
            </button>
          </div>
                  ...
                  <div key={job.jobId} className="flex gap-3 p-3 bg-[#0D1321] rounded-xl">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-1 mb-0.5">
                        <p className="text-white text-xs font-semibold truncate leading-tight">{job.title}</p>
                        <span className="text-[#94A3B8] text-[10px] flex-shrink-0">
                          {formatTimeAgo(job.savedAt)}
                        </span>
                      </div>
                      <p className="text-[#94A3B8] text-xs mb-1.5 truncate">
        ...
        {/* Right Panel — full width on mobile, stacks naturally */}
        <div className="space-y-4">
          {/* Status breakdown */}
          <div className="bg-[#111827] border border-white/10 rounded-xl p-4">
            <p className="text-[#00D4AA] text-[10px] font-bold tracking-widest mb-3">
          ...
          <div className="bg-[#111827] border border-white/10 rounded-xl p-4">
            ...
            {/* 2-col grid on mobile for quick actions */}
            <div className="grid grid-cols-2 sm:grid-cols-1 gap-2">
              {[
                ...
                <button
                  key={item.path}
                  onClick={() => navigate(item.path)}
                  className="flex items-center gap-2 p-2 rounded-lg bg-[#0D1321] hover:bg-white/5 transition text-left"
                >
                  <span className="text-sm">{item.icon}</span>
                  <span className="text-[#94A3B8] text-xs hover:text-white truncate">{item.label}</span>
                  <span className="text-[#94A3B8] text-xs ml-auto hidden sm:inline">›</span>
                </button>
              ))}
            </div>
          </div>
